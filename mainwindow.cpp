...
void MainWindow::RestartCap(QString url)
{
    capture.release();
    capture.open(url.toUtf8().constData());
    if(!capture.isOpened()){
        return;
    }
    FrameUpdate();
...
{
    cvtColor(Subtracted,RGBOuted,COLOR_GRAY2BGR);
    bitwise_and(BGRSource,RGBOuted,RGBOuted);
    cvtColor(RGBOuted,RGBOuted,COLOR_BGR2RGB);
}
...
void MainWindow::GenMenuStream(QMenu *Stream){

    QWidgetAction *MenuStreamAction = new QWidgetAction(this);
    QLineEdit *URLStream = new QLineEdit("udp://@127.0.0.1:1400", this);
    MenuStreamAction->setDefaultWidget(URLStream);
    connect(MenuStreamAction, SIGNAL(returnPressed()), this, SLOT(ChangeURL(URLStream)));

    Stream->addAction(MenuStreamAction);
}

void MainWindow::ChangeURL(QLineEdit *Stream){
    RestartCap(Stream->text());
}
...
vector<vector<Point>> MainWindow::GetContoursFromFrame(Mat& InputBinMat)
{
    ...
    /// Find contours
    findContours(InputBinMat, contours, hierarchy, RETR_TREE, CHAIN_APPROX_SIMPLE, Point(0, 0));
    return contours;
}
